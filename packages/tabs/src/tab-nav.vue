  ...
  export default {
    ...
    render(h) {
      ...
      const scrollBtn = scrollable
        ? [
          <span class={['zz-tabs__nav-prev', scrollable.prev ? '' : 'is-disabled']} on-click={scrollPrev}><i class="zz-icon-arrow-left"></i></span>,
          <span class={['zz-tabs__nav-next', scrollable.next ? '' : 'is-disabled']} on-click={scrollNext}><i class="zz-icon-arrow-right"></i></span>
        ] : null;
      ...
      const tabs = this._l(panes, (pane, index) => {
        ...
        const btnClose = closable
          ? <span class="zz-icon-close" on-click={(ev) => { onTabRemove(pane, ev); }}></span>
          : null;
      ...
      return (
        <div class={['zz-tabs__nav-wrap', scrollable ? 'is-scrollable' : '', `is-${ this.rootTabs.tabPosition }`]}>
          {scrollBtn}
          <div class={['zz-tabs__nav-scroll']} ref="navScroll">
            <div
              class={['zz-tabs__nav', `is-${ this.rootTabs.tabPosition }`, stretch && ['top', 'bottom'].indexOf(this.rootTabs.tabPosition) !== -1 ? 'is-stretch' : '']}
              ref="nav"
              style={navStyle}
              role="tablist"
