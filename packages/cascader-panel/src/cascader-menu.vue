...
export default {
  ...
  methods: {
    ...
    renderEmptyText(h) {
      return (
        <div class="zz-cascader-menu__empty-text">{ this.t('el.cascader.noData') }</div>
      );
    },
    renderNodeList(h) {
      const { menuId } = this;
      ...
      return [
        ...nodes,
        isHoverMenu ? <svg ref='hoverZone' class='zz-cascader-menu__hover-zone'></svg> : null
      ];
    }
  },
  ...
  render(h) {
    ...
    return (
      <zz-scrollbar
        ...
        role="menu"
        id={ menuId }
        class="zz-cascader-menu"
        wrap-class="zz-cascader-menu__wrap"
        view-class={{
          'zz-cascader-menu__list': true,
          'is-empty': isEmpty
        }}
        { ...events }>
